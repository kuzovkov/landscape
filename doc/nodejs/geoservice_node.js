...
app.get('/nearest',function(req,res){
	...
	getNearest([lat_point,lng_point], db,scale, function(result){
		res.writeHead(200, {"Content-Type": "text/html","Access-Control-Allow-Origin": "*"});
...
/*поиск маршрута*/
function getRoute(from, to, db,scale, callback){
	getNodeId(from, db, scale, function(start){
	     getNodeId(to, db, scale, function(end){
...
function getNearest(point, db,scale, callback){
	getNodeId(point, db, scale, function(node_id){
		var sql = 'SELECT AsGeoJSON(geometry) AS geometry FROM roads_nodes WHERE node_id='+node_id+' LIMIT 1';
		db.spatialite(function(err) {
				db.get(sql, function(err, row) {
				//console.log(JSON.stringify(row));
					var coords = null;
...
* @return id узла 
**/
function getNodeId(dot, db, scale, callback){
	var sector = latlng2sector(dot[0],dot[1],scale);
	var sql = "select node_id, MIN(Pow(("+dot[1]+"-X(geometry)),2) +Pow(("+dot[0]+"-Y(geometry)),2)) as rast from roads_nodes where connected=1 and sector="+sector;
	db.spatialite(function(err) {
		db.get(sql, function(err, row) {
            //console.log(JSON.stringify(row));
			if ( row != undefined ){
